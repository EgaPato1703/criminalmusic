...
const WelcomePage = () => {
  const { loginWithTelegram, setupUserRole, loading } = useAuth();
  ...
  console.log('Welcome page loaded, loading state:', loading);
    ...
    <WelcomeContainer>
      ...
      {selectedRole && (
        <SubmitButton
          ...
          disabled={loading || (selectedRole === 'artist' && !artistData.pseudonym)}
          whileHover={{ scale: 1.02 }}
          whileTap={{ scale: 0.98 }}
        ...
        >
          {loading ? 'Вход в систему...' : 'Войти в Criminal Music'}
        </SubmitButton>
      )}
    </WelcomeContainer>
  );
